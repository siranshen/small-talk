...
const constructSystemPrompt = (language: string, evalLanguage: string, convo: string) => {
  ...
  2. Evaluate user's messages *one by one*. Note: only evaluate user's; must skip assistant's.
  3. Analyze the grammar, vocabulary usage, fluency of user's messages. Note: user's text is generated by speech recognition, so it may be filled with punctuation mistakes and filler words.
  4. Exclude punctuation mistakes and filler words, and then list all the other mistakes you find, each followed by a suggestion.
